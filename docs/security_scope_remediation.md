...
| SEC-003 | High | In Progress | Brain sends `escalation_alert` but edge node has no handler | Brain: `atlas_brain/escalation/evaluator.py:227`; Edge handlers: `atlas_node/main.py:194-203` | `atlas_node/main.py` |
...
- Active: `SEC-003` (add `escalation_alert` handling on node)
